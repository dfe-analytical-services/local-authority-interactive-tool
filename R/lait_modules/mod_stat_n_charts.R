...
StatN_Chart_Input2UI <- function(id) {
  ns <- NS(id)
...
StatN_Chart_Input2Server <- function(id, la_input, stat_n_long, shared_values) {
  moduleServer(id, function(input, output, session) {
    # Helper function to retain only the valid selections that are in the available choices
    retain_valid_selections <- function(current_choices, previous_selections) {
      intersect(previous_selections, current_choices)
    ...
    # Return the selected inputs as reactive values for use elsewhere in the app
    list(
      line_input = reactive(input$chart_line_input),
      bar_input = reactive(input$chart_bar_input)
    )
  })
}


StatN_MultiChartsUI <- function(id) {
  ns <- NS(id)
        ...
        bslib::card_body(
          bslib::layout_sidebar(
            sidebar = bslib::sidebar(
              title = "Filter options",
              position = "left",
              StatN_Chart_Input2UI(
                ns("chart_input") # Line chart input only
              )[[1]]
            ),
...
StatN_MultiChartsServer <- function(id,
                                    app_inputs,
                                    bds_metrics,
                                    stat_n_la,
                                    shared_values) {
  moduleServer(id, function(input, output, session) {
    ...
    # Pulling specific choices available for selected LA & indicator
    line_input <- StatN_Chart_Input2Server(
      "chart_input",
      app_inputs$la,
      stat_n_long,
      shared_values
    ...
    # Pulling specific choices available for selected LA & indicator
    bar_input <- StatN_Chart_Input2Server(
      "chart_input",
      app_inputs$la,
      stat_n_long,
      shared_values
    ...
    output$line_chart <- ggiraph::renderGirafe({
      ...
      stat_n_line_chart_data <- stat_n_long() |>
        # Filter for random areas - simulate user choosing up to 6 areas
        dplyr::filter(
          (`LA and Regions` %in% line_input()) |
            (`LA and Regions` %in% app_inputs$la())
        ) |>
        # Set area orders so selection order starts on top of plot
        reorder_la_regions(
          rev(intersect(c(app_inputs$la(), line_input()), valid_regions)),
          after = Inf
        )
    ...
    output$bar_chart <- ggiraph::renderGirafe({
      ...
      stat_n_bar_multi_data <- stat_n_long() |>
        # Filter for random areas - simulate user choosing up to 6 areas
        dplyr::filter(
          (`LA and Regions` %in% bar_input()) |
            (`LA and Regions` %in% app_inputs$la())
        ) |>
        # Set area orders so selection order starts on top of plot
        reorder_la_regions(
          intersect(c(app_inputs$la(), bar_input()), valid_regions)
        )
