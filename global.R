...
bds_clean <- bds |>
  dplyr::mutate(values_clean = dplyr::case_when(Values == "-" ~ NA,
                                                Values == "c" ~ NA,
                                                is.na(Values) ~ NA,
                                                T ~ Values),
...
bds_metrics <- metrics_clean |>
  dplyr::select(Topic, Measure_code, Measure, Measure_short,
...
############### Test to see if the above are the same
